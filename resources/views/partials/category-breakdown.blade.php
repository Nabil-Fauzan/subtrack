...
<div class="rounded-3xl bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800/80 p-6 sm:p-7 shadow-xl backdrop-blur-md transition-colors duration-200">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-5 gap-4">
        <div>
            ...
            <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Proporsi pengeluaran bulanan berdasarkan kategori langganan aktif</p>
        </div>
        <div class="text-right sm:text-left">
            <span class="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Total Terdistribusi:</span>
            <span class="text-sm font-bold text-indigo-600 dark:text-indigo-400 ml-1">Rp {{ number_format($metrics['total_monthly_cost'], 0, ',', '.') }}/bln</span>
        </div>
    </div>

    <!-- Multi-Color Segmented Bar -->
    <div class="w-full h-4 bg-zinc-100 dark:bg-zinc-950 rounded-full overflow-hidden flex p-0.5 border border-zinc-200 dark:border-zinc-800">
        @forelse($categoryBreakdown as $cat)
            ...
            ></div>
        @empty
            <div class="w-full h-full bg-zinc-200 dark:bg-zinc-800 rounded-full"></div>
        @endforelse
    </div>

    <!-- Category Legend Items -->
    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-4 border-t border-zinc-200 dark:border-zinc-800/60">
        @forelse($categoryBreakdown as $cat)
            <div class="flex items-start space-x-3 p-2.5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800/40 hover:border-zinc-300 dark:hover:border-zinc-700/60 transition-all">
